...
class Blocks {
  ...
  public summarizeBlock(block: IBitcoinApi.VerboseBlock): BlockSummary {
    ...
    const stripped = block.rawtx.map((tx: IBitcoinApi.VerboseTransaction) => {
      return {
        txid: tx.txid,
        vsize: tx.weight / 4,
  ...
  public async $updateBlocks(): Promise<number> {
    ...
    if (!this.lastDifficultyAdjustmentTime) {
      ...
      if (blockchainInfo.blocks === blockchainInfo.headers) {
        const heightDiff = blockHeightTip % 1; // #FLZ_CHANGE 2016
        ...
        if (blockHeightTip >= 1) { // #FLZ_CHANGE 2016
          const previousPeriodBlockHash = await bitcoinApi.$getBlockHash(blockHeightTip - heightDiff - 1); // #FLZ_CHANGE 2016
    ...
    while (this.currentBlockHeight < blockHeightTip) {
      ...
      if (block.height % 1 === 0) { // #FLZ_CHANGE 2016
        if (Common.indexingEnabled()) {
          let adjustment;
          if (['liquid', 'liquidtestnet'].includes(config.MEMPOOL.NETWORK)) {
            adjustment = NaN;
  ...
  public async $getStrippedBlockTransactions(hash: string, skipMemoryCache = false, skipDBLookup = false, cpfpSummary?: CpfpSummary, blockHeight?: number): Promise<TransactionClassified[]>
  {
  ...
  public async $indexCPFP(hash: string, height: number, txs?: MempoolTransactionExtended[]): Promise<CpfpSummary | null> {
    ...
    if (!transactions) {
      ...
      if (!transactions) {
        const block = await bitcoinClient.getBlock(hash, 2);
        transactions = block.rawtx.map(tx => {
          tx.fee *= 100_000_000;
          return tx;
        });
      }
