...
class PriceUpdater {
  ...
  public async $run(): Promise<void> {
    ...
    try {
      ...
      logger.err(`Cannot save FLC prices in db. Reason: ${e instanceof Error ? e.message : e}`, logger.tags.mining);
    }
  ...
  /**
   * Fetch last FLC price from exchanges, average them, and save it in the database once every hour
   */
  private async $updatePrice(): Promise<void> {
    ...
    for (const currency of this.currencies) {
      ...
      for (const feed of this.feeds) {
        ...
        if (feed.currencies.includes(currency)) {
          try {
            let price = await feed.$fetchPrice(currency); 
            price = price / 10000000;  // #FLZ_CHANGE
            price = Math.round(price * 10000) / 10000;

            if (price > -1 && price < MAX_PRICES[currency]) {
              prices.push(price);
            }
            logger.debug(`${feed.name} FLC/${currency} price: ${price}`, logger.tags.mining);
          } catch (e) {
            logger.debug(`Could not fetch FLC/${currency} price at ${feed.name}. Reason: ${(e instanceof Error ? e.message : e)}`, logger.tags.mining);
          }
        }
      }
      if (prices.length === 1) {
        logger.debug(`Only ${prices.length} feed available for FLC/${currency} price`, logger.tags.mining);
      }
      ...
      if (prices.length === 0) {
        ...
        // this.latestPrices[currency] = Math.round(getMedian(prices));  // #FLZ_CHANGE
    ...
    logger.info(`Latest FLC fiat averaged price: ${JSON.stringify(this.latestPrices)}`);
  ...
  private async $insertMissingRecentPrices(type: 'hour' | 'day'): Promise<void> {
    ...
    for (const time in grouped) {
      ...
      for (const currency in grouped[time]) {
        ...
        prices[currency] = prices[currency] / 10000000;  // #FLZ_CHANGE
        prices[currency] = Math.round(prices[currency] * 10000) / 10000;
